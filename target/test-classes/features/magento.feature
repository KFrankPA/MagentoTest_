...
Feature: Add products to cart and check cart total
  ...
  Scenario: Add products to cart and check cart total
    ...
    When I add "Gwyn Endurance Tee Medium Green" to the cart with quanitiy "4"
    And I select "United Kingdom" as the shipping address
    Then the cart total should be "$92.00"
    When I update the quantity of "Gwyn Endurance Tee Medium Green" to "3"
    And I add to quantity "1" "Gwyn Endurance Tee Small Yellow" to chart
